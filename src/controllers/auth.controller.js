...
const register = asyncHandler(async (req, res) => {
  ...
  // 1) allowlist: ايميلات تتحول Admin تلقائياً
  ...
  const roleToCreate = isAdminEmail ? "admin" : "owner";

  // 2) منع التسجيل العام: اسمح فقط للـ Admin (توكن) بإنشاء Owner
  // لكن اسمح للإيميل الموجود في allowlist بإنشاء Admin حتى لو ما في توكن (لأول أدمن)
  if (!env.ALLOW_PUBLIC_REGISTER && !isAdminEmail) {
