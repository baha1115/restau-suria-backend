...
// ✅ NEW: POST /api/owner/items/:itemId/image
...
const uploadItemImage = asyncHandler(async (req, res) => {
  ...
  const uploaded = await uploadBufferToCloudinary(req.file.buffer, {
    folder: `${env.CLOUDINARY_FOLDER}/restaurants/${restaurant.slug}/items`
  });

  item.imageUrl = uploaded.secure_url;
  await item.save();

  return ok(res, { itemId: item._id, imageUrl: item.imageUrl }, "Item image uploaded");
});
